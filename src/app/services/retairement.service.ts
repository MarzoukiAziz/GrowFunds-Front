...
export class RetairementService {
  ...
  getData(
    ...
    const url1 = this.api_url + '/retirementPlan?'+queryString;
    const url2 = this.api_url + '/retirementPlan?'+queryString;

    return this._http.get<Map<string, number>[]>(url1);
  }
  getData2(
    currentAge:number,
    initialNetWorth: number,
    ...
    const queryString = params.toString();
    const url2 = this.api_url + '/retirementPlan2?'+queryString;

    return this._http.get<number[]>(url2);
  }
